...
static int addNode(LinkedList* this, int nodeIndex,void* pElement)
{
	...
	if(this!=NULL && auxNode!=NULL && nodeIndex>=0 && nodeIndex<=ll_len(this))
	{
		...
		{
			auxNodePrevio = getNode(this,nodeIndex-1);
			if(auxNodePrevio != NULL)
			{
				auxNode->pNextNode = auxNodePrevio->pNextNode;
				auxNodePrevio->pNextNode = auxNode;
				this->size++;
				returnAux=0;
			}
		}
...
int ll_remove(LinkedList* this,int index)
{
	...
	if(this!=NULL && index>=0 && index<ll_len(this))
	{
		...
		{
			auxNodePrevio=getNode(this,index-1);
			if(auxNodePrevio!=NULL && auxNode!=NULL)
			{
				auxNodePrevio->pNextNode = auxNode->pNextNode;
				returnAux=0;
			}
		}
		if(returnAux==0)
...
int ll_indexOf(LinkedList* this, void* pElement)
{
    ...
    if(this!=NULL)
    {
		for(int i=0; i<ll_len(this); i++)
		{
			...
			if(auxElement==pElement)//(auxNode!=NULL && (auxNode->pElement==pElement)) //auxElement==pElement
			{
				returnAux=i;
				break;
			}
	...
	if(this!=NULL && pFunc!=NULL && (order==0 || order==1))
	{
		for(int i=0; i<ll_len(this)-1; i++)
		{
			for(int j=i+1; j<ll_len(this); j++)
			{
				...
				if(((pFunc((aux),(aux2))>0) && order==1) || ((pFunc((aux),(aux2))<0) && order==0))
				{
